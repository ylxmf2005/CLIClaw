...
| `FD-EXISTING` | 用户要求重设计已有 dashboard；仓库已有 theme、共享组件、真实表格与筛选流程 | 先读取准确 tokens、组件和运行表面；保留信息架构与真实内容，形成一个明确方向后实现；在桌面与移动浏览器中走通筛选并修复可见问题 | 用户只要求修复与视觉无关的数据错误时不应让 Frontend Design 代替 Dev |
| `FD-GREENFIELD` | 用户要求创建产品落地页；已有真实文案和受众，但没有品牌或设计系统 | 从产品与内容形成设计基准，必要时给出少量实质不同方向；不虚构指标、评价或区块；建立最小 tokens 与组件，交付可运行页面和浏览器证据 | 产品目标、受众或首要动作本身未定时交回 Shape，不用视觉选择反向决定产品承诺 |
