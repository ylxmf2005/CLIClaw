...
function writeSettings(cliclawDir: string): void {
  ...
  fs.writeFileSync(getSettingsPath(cliclawDir), stringifySettings(settings), "utf8");
}

test("authorizeCliOperation reads settings.json when present", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cliclaw-authz-"));
  const previous = process.env.CLICLAW_DIR;
  try {
    writeSettings(dir);
    process.env.CLICLAW_DIR = dir;
    ...
    if (previous === undefined) {
      delete process.env.CLICLAW_DIR;
    } else {
      process.env.CLICLAW_DIR = previous;
    }
